...
**Team names** — Italian save disks carry team names in `PM1.nam` (44 × 20-byte records). English/BETA save disks don't ship `PM1.nam`; `SaveSlot` initialises with `"Team 0".."Team 43"` placeholders and sets `team_names_from_save=False`. When a game disk is loaded, `SaveSlot.apply_team_name_fallback(gd.team_names)` replaces the placeholders with names extracted from the game disk's `start.dat` (44 × 100-byte team records; NUL-terminated name at sub-offset 0x3C; slot 43 is template garbage and stays a placeholder). `GameDisk.team_names` is populated for any PM-custom-file-table disk where `start.dat` parses cleanly. The fallback is a no-op when the save disk already had real names, so it's safe to apply unconditionally. GUI applies it in both directions (open save → open game, or open game → open save); CLI applies it inside `_load_game_disk(path, slot)`.
