...
- Compare to: shows dashed arrows from a reference zone's positions to the current ones, so you can see how each shirt moves between the two snapshots. Default `(previous zone)` auto-follows the last zone you left; pick a specific zone to pin it, or `(none)` to hide the overlay. A legend at the bottom-right of the pitch names the current reference zone.
