...
3. The tool decompresses the game executable, extracts all 245 Italian surnames, and
...
table (245 Italian names) is stored in the DEFAJAM-packed `2507` executable on the game
disk, at offset `$15B02` in the decompressed image. The hash algorithm was reverse-engineered
...
### Teams (Italian version)

The 44 teams span Serie A through Serie D and are stored in `PM1.nam` (44 × 20-byte
records, null-terminated ASCII). In the Italian version these include Milan, Sampdoria,
Fiorentina, Napoli, Roma, Torino, and 38 further clubs.
